...
CI2<-cbind(lower_quantile2,upper_quantile2)
print(CI2)
```
## Question 3
...
plot(MCMC.out$sigmaSq.chain[,1], xlab = "Iteration", ylab = "sigmaSq1", type = "l")
```
### D
```{r}
N<-10
...
credible_interval<-matrix(NA,2,11)

for(i in 1:10)
{
  credible_interval[,i] <- quantile(MCMC.out$theta.chain[,i], c(0.025, 0.975))
}
credible_interval[,11]<-quantile(MCMC.out$m.chain, c(0.025, 0.975))
...
We can take the 2.5% quantile as the lower boundary and 97.5% quantile as the upper boundary of the credible intervals.